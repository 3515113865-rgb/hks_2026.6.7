...
st.set_page_config(page_title="YOLO11 Live Detector", page_icon="🧠", layout="centered")
st.title("🧠 YOLO11 - Image & Video (Live)")
...
if mode == "Image":
    ...
    with st.spinner(f"Running {task} on image..."):
        ...
        annotated = result.plot(pil=True)  # Works for detect/seg/pose
...
# -----------------------
    ...
    source_radio = st.radio("Video Source", ["Use Default Video", "Upload a Video"], horizontal=True)
    
    input_path = None
    video_name = ""
    
    if source_radio == "Upload a Video":
        ...
        # Use default video
        default_video_path = os.path.join("resources", "videos", "video.mp4")
        if not os.path.exists(default_video_path):
            st.error(f"Default video not found at {default_video_path}")
            st.stop()
    ...
    st.caption(f"Input file: {video_name}  •  Task: {task}  •  Model: {model_path}")
    ...
    if start:
        ...
        with st.spinner(f"Running {task} on video (live)..."):
            while cap.isOpened():
                ...
                annotated_bgr = results.plot()  # ndarray (BGR), works for detect/seg/pose
